# portfolio-bounds-enforcement Specification

## Purpose
Fail-closed, deterministic validation of the Final Strategist's published output. The
Flow re-prices the final output via `PortfolioAllocatorTool` (never trusting LLM-written
numeric fields) and validates the result against per-risk-profile allocation, stop-distance,
R/R, trade-setup, and bucket-sum invariants before `run_analysis()` returns — raising
`BoundsViolationError` rather than publishing a non-compliant result.
