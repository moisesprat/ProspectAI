# Capability Spec: Critic Compact Context

## Overview

This spec defines the requirements for the Critic agent's context assembly in `ProspectAIFlow`. The Critic receives a compact per-ticker reference table (only the fields its checklist actually verifies) instead of the full upstream slim outputs, keeping its context focused and reducing token usage.
...
### Requirement: Final Strategist context is scoped to Draft and Critic outputs only
The `final_strategy()` flow method SHALL build its context from two inputs only: the draft strategy output (`_slim_draft()`) and the Critic's output (`_slim_critique()`). The three upstream slim outputs (`_slim_market_for_strategy()`, `_slim_technical()`, `_slim_fundamental()`) SHALL NOT be included. The Draft already synthesizes all upstream data; the Final Strategist's job is solely to apply the Critic's directives to the Draft.

#### Scenario: Final Strategist context excludes upstream slim outputs
- **WHEN** `final_strategy()` assembles the task description context
- **THEN** the context SHALL include exactly two sections: Draft Strategy Output and Critic Review Output
