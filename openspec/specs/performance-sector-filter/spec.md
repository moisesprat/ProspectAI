# Spec: Performance Sector Filter

## Purpose
The Performance tab on `stats.html` provides a sector filter dropdown that lets users narrow KPI cards and the track record table to a specific sector, or view all sectors at once.
