# Capability Spec: Execution Metrics

## Overview

This spec defines requirements for execution metrics tracking in `ProspectAIFlow`. Every pipeline run SHALL produce a structured `execution_metrics` dict containing wall-clock timing and LLM token counts per phase, aggregated totals, and a per-model breakdown. Metrics are surfaced in the CLI (printed to stdout) and in the web UI (forwarded via SSE and rendered in the report section).

Token counts are read directly from `CrewOutput.token_usage` after each `await crew.akickoff()` call — no LiteLLM callbacks are used. A fresh `ExecutionTracker` instance is created per `run_analysis()` call to prevent cross-run accumulation.
...
### Requirement: Web UI receives execution_metrics via SSE and renders it
When `run_analysis()` is called with a `progress_callback`, the callback SHALL be invoked once at pipeline completion with an event dict containing `event: "execution_complete"` and `metrics: <execution_metrics dict>`. The backend SSE stream SHALL include `metrics` in the `pipeline_done` event. The frontend SHALL render an Execution Metrics section in the report showing pipeline time, per-phase table (elapsed, input, output, cached, total tokens), and a by-model breakdown.
...
#### Scenario: SSE pipeline_done event includes metrics
- **WHEN** `serve.py` emits a `pipeline_done` SSE event
- **THEN** the event dict SHALL contain a `metrics` key with the full `execution_metrics` dict

#### Scenario: Report section shows execution metrics
- **WHEN** the frontend receives a `pipeline_done` event with a non-null `metrics` field
- **THEN** the report body SHALL display an "Execution Metrics" section with pipeline time, per-phase rows, and per-model rows
